...
With even this first attempt using a test data set on a single locus (LSU), intriguing patterns are already becoming apparent that violate many assumptions of convenience often made by researchers. 
Metabarcoding analysis pipelines and distance-based clustering usually assume that the entirety of a large group has a similar inter-specific distance, such as 0.03 PID for fungi ITS sequence. 
Here we observe that the values for optimal inter-specific K2 thresholds of genera easily range from 0.0 to 0.1 (Figure \ref{fig:threshold} \subref{fig:optimal_threshold_distribution_K2_Species}). 
The inter-genus distance covers twice that range  (Figure \ref{fig:threshold} \subref{fig:optimal_threshold_distribution_K2_Genus}). 
...
This means that studies using LSU for all fungi at a single clustering threshold are often incorrectly differentiating species up to 50\% of the time (Figure \ref{fig:error} \subref{fig:optimal_error_boxplot_K2_Species}).
...
There are also many ways the pipeline could be made faster using recursion that would make it more useful for large analyses. 
...
\begin{figure}[ht]
 ...
 \caption[fig:error]{%
  Error rates at optimal thresholds for differentiating species [\subref{fig:optimal_threshold_boxplot_K2_Species} \subref{fig:optimal_threshold_distribution_K2_Species}, \subref{fig:optimal_threshold_tree_K2_Species}] and genera [\subref{fig:optimal_threshold_boxplot_K2_Genus} \subref{fig:optimal_threshold_distribution_K2_Genus}, \subref{fig:optimal_threshold_tree_K2_Genus}]. For figures \subref{fig:optimal_threshold_tree_K2_Species} and \subref{fig:optimal_threshold_tree_K2_Genus} red is 100\%, green is 50\%, and blue is 0\%.}
 \label{fig:error}
\end{figure}
