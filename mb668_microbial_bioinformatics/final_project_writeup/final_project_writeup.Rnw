...
Amplicon metagenomics, also known as metabarcoding, is a rapidly advancing field that has great potential for characterizing the microbial communities.
Before metabarcoding, scientist were restricted to low-throughput techniques heavily biased by culturability. 
...
Metabarcoding allows for up to hundreds of samples to be exhaustively characterized using a single Illumina MiSeq run, costing less than \$3000. 
With the ever decreasing cost of sequences and increasing reads lengths of high-throughput sequences like Illumina, metabarcoding is likely to become a routine technique in microbial ecology laboratories. 
...
Therefore I am developing a set of python and R scripts to characterize taxon-specific statistics relevant to metabarcoding.
The pipeline can combine and reformat multiple different reference databases, randomly sub-sample the combined database by taxon, and calculate the optimal distance clustering threshold and predicted error rates using an arbitrary distance matrix. 
...
My scripts use reference database sequences to produce useful tables of statistics pertaining to each taxon and intuitive, information-rich graphics for evaluation of novel experimental protocols.

\section*{Methods}

\subsection*{Database consolidation and standardization}

In planning a amplicon metabarcoding experiment it is sometimes necessary to combine multiple reference database with different formats so they can be used in the same pipeline.
...
This means that the first step in any pipeline that seeks to use sequences from multiple reference databases must be able to read in multiple format.
...
Taxa are differentiated by their linage (i.e. the name of the taxon, as well all of the higher level taxa it is a part of). 
...
This allows it to reads the database one sequences at a time and thus it is able to process nearly any size of reference database of data with minimal RAM.
...
In addition, more conventional graphs displaying the distribution of statistics at each taxonomic level are also produced. 
...
To demonstrate the abilities of this pipeline, I analyzed the Ribosomal Database Project (RDP) LSU reference multiple sequence alignment, testing it ability to differentiate a range of taxonomic levels from species to class. 
...
After appending genus and species information (if present in the organism name) onto the taxonomy string during formatting, 37,733 distinct taxa were present. 
...
These are displaying the information form which the optimal threshold and error rate were calculated. 

The average optimal clustering threshold for differentiating species is nearly equal across taxonomic levels at around 0.01 (Figure \ref{fig:threshold} \subref{fig:optimal_threshold_boxplot_K2_Species}. 
...
With even this first attempt using a test data set on a single locus (LSU) intriguing patterns are already becoming apparent that violate many assumptions made by researchers out of convenience. 
Metabarcoding analysis pipelines and used distance-based clustering usually assume that the entirety of a large group has a similar inter-specific distance, such as 3\% PID for fungi ITS sequence. 
...
There are also many ways the pipeline could be seeded up using recursion that would make it more useful for large analyses. 
