...
summary_graph <- function(data, summarized, title, x_label, y_label, var) {
  # X density plots --------------------------------------------------------------------------------
  x_density <- ggplot(data, aes_string(x = var, color = "site", fill = "site")) +
  ...
  gt <- ggplot_gtable(ggplot_build(x_density))
  ge <- subset(gt$layout, name == "panel")
  x_density <- gt[ge$t:ge$b, ge$l:ge$r]
  # Main scatter plot ------------------------------------------------------------------------------
  xy_scatter <- ggplot(data, aes_string(x = var, y = "rich", color = "site")) +
    ...
    theme(panel.grid = element_blank(), 
          ...
          panel.background = element_blank())
  browser()
  gt <- ggplot_gtable(ggplot_build(xy_scatter))
  xy_scatter <- ggplotGrob(xy_scatter)
  # Y density plots --------------------------------------------------------------------------------
  y_density <- ggplot(data, aes_string(x = "rich", color = "site", fill = "site")) +
    geom_density(alpha = .2, position = "identity", adjust = .8) +
    coord_flip() +
    scale_x_continuous(expand = c(0, 0)) +
#     coord_cartesian(ylim=c(0,100), xlim = range(data[[var]])) +
    guides(color = FALSE, fill = FALSE) +
    theme(panel.grid = element_blank(), 
          axis.ticks = element_blank(), 
          axis.text  = element_blank(), 
          ...
          panel.background = element_blank())
  gt <- ggplot_gtable(ggplot_build(y_density))
  ge <- subset(gt$layout, name == "panel")
  y_density <- gt[ge$t:ge$b, ge$l:ge$r]
  #   upViewport()
  ...
  pushViewport(viewport(layout=grid.layout(nrow = 2, ncol = 2,
                                           widths = unit(c(0.8, 0.2), "null"),
                                           heights = unit(c(0.2, 0.8), "null"))))
...
#     print(my_plot, newpage = FALSE)
  ...
  draw_plot(1, 1, x_density)
  draw_plot(2, 1, xy_scatter)
  draw_plot(2, 2, y_density)
  }
