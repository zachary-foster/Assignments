...
library(scales)
library(boot)
opts_chunk$set(fig.width = 7, fig.height = 5, message = FALSE, warning = FALSE, echo = FALSE)
```
...
data_1$site <- paste("Site", data_1$site)
data_2$site <- paste("Site", data_2$site)
data_1$site <- factor(data_1$site, ordered = TRUE, levels = rev(data_1$site))
data_2$site <- factor(data_2$site, ordered = TRUE, levels =  c("Site 5", "Site 3", "Site 2"))
```
...
cqq_scatter <- function(data, cat_1, quant_1, quant_2, title = "", x_label = quote(quant_1),
  ...
  plot_text <- cbind(ddply(data, "site", function(x) lm_eqn(x[[quant_1]], x[[quant_2]])),
                     ddply(data, "site", function(x) mean(range(x[[quant_1]])))[2],
                     ddply(data, "site", function(x) mean(c(mean(range(x[[quant_2]])), max(x[[quant_2]]))))[2])
  names(plot_text) <- c("site","text", "x", "y")
  plot_text$site <- factor(plot_text$site, ordered = TRUE, levels = levels(data$site)) #makes same order as in data
  small_scatters <- ggplot(data, aes_string(x = quant_1, y = quant_2, color = "site")) +
    geom_smooth(method = lm, fill = "#FFFFFFFF") +
    geom_point() +
    geom_text(data = plot_text, aes(x = x, y = y, label = text), color = "#444444",  parse = TRUE, size = 3, fontface = "bold") +
  ...
  pushViewport(viewport(layout=grid.layout(nrow = 4, ncol = 4,
                                           widths = unit(c(0.03, 0.03, 0.8, 0.16), "null"),
