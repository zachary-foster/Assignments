...
compare_dist <- function(expected, df) {
  ...
  p_value <- 1 - pchisq(sum((comp$observed-comp$expected)^2/comp$expected), nrow(comp) - df)
  print(paste("Chi-squared p-value:", signif(p_value)))
  return(comp)  
}
comp <- compare_dist(poisson, 1)
```
...
Before significance testing, larvae count categories with low values were summed such that their summed count was greater than 2.
Judging from both visual inspection of the observed distribution compared to the predicted Poisson distribution as well as the chi-squared test results ($p=1.49\cdot10^{-13}$), I believe that the Poisson is not a good fit for the observed data.
...
The values look much more similar to the observed values and the chi-squared test for a difference between the two distributions was insignificant ($p=0.799$), indicating that there is no reason to exclude the negative binomial as a potential model. 
